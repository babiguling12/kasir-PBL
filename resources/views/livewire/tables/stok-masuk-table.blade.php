<div>
    <div
        ...
        <div class="w-full md:w-1/2">
            <form class="flex items-center">
                <label for="search-stok-masuk" class="sr-only">Search</label>
                <div class="relative w-full">
                    ...
                    <input type="text" wire:model.live.rebounce.100ms="search" id="search-stok-masuk"
                        name="search-stok-masuk" placeholder="Search for stok-masuk"
        ...
        <div
            ...
            @if (auth()->user()->role === 'admin')
                <button type="button" wire:click="$dispatch('openModal', { component: 'modals.stok-masuk-modal' })"
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                    @forelse ($stok_masuk as $stok_msk)
                    <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                        ...
                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <span
                                class="bg-primary-100 text-primary-800 text-xs font-medium px-2 py-0.5 rounded dark:bg-primary-900 dark:text-primary-300">{{ $stok_msk->produk->barcode }}</span>
                        </td>
                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $stok_msk->produk->nama_produk }}</td>
                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center">
                            {{ $stok_msk->jumlah }}</td>
                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $stok_msk->supplier->nama }}</td>
                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="flex items-center">
                                <span class="text-gray-500 dark:text-gray-400 ml-1">{{ $stok_msk->keterangan }}</span>
                            </div>
                        </td>
                        ...
                        <td
                            ...
                            <div class="flex items-center space-x-4">
                                @if (auth()->user()->role === 'admin')
                                    <button type="button"
                                        wire:click="$dispatch('openModal', { component: 'modals.stok-masuk-modal', arguments: {stokMasuk: {{ $stok_msk->id }} }})"
